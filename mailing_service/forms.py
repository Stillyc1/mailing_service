...
class MailingForm(ModelForm):
    ...
    class Meta:
        model = Mailing
        exclude = ('date_start', 'date_end', 'status',)
    ...
    def clean(self):
        ...
        for word in banned_words:
            if word in email.lower():
                self.add_error('email', f'Нельзя использовать слово "{word.title()}" в почтовом адресе.')
            if word in fullname.lower():
                self.add_error('fullname', f'Нельзя использовать слово "{word.title()}" в ФИО.')
            if word in comment.lower():
                self.add_error('comment', f'Нельзя использовать слово "{word.title()}" в подробной информации.')
    ...
    def clean(self):
        ...
        for word in banned_words:
            if word in head_letter.lower():
                self.add_error('head_letter', f'Нельзя использовать слово "{word.title()}" в теме письма.')
            if word in body_letter.lower():
                self.add_error('body_letter', f'Нельзя использовать слово "{word.title()}" в содержании письма.')
